"""Assignment 1 - Domain classes (Task 2)

CSC148, Winter 2021

This code is provided solely for the personal and private use of
students taking the CSC148 course at the University of Toronto.
Copying for purposes other than this use is expressly prohibited.
All forms of distribution of this code, whether as given or with
any changes, are expressly prohibited.

Authors: Diane Horton, Ian Berlott-Atwell, Jonathan Calver,
Sophia Huynh, Myriam Majedi, and Jaisie Sin.

All of the files in this directory and all subdirectories are:
Copyright (c) 2021 Diane Horton, Ian Berlott-Atwell, Jonathan Calver,
Sophia Huynh, Myriam Majedi, and Jaisie Sin.

===== Module Description =====
===== Module Description =====

This module contains the classes required to represent the entities
in the simulation: Parcel, Truck and Fleet.
"""
from typing import List, Dict
from distance_map import DistanceMap


class Parcel:
    # It must be consistent with the Fleet class docstring examples below.
    """
    Parcel object initialize with its id, wheres its sent, destination and its
    volume
    ===== Public Attributes =====
    parcel_id: Each unique id, no parcel ID occurs more than once in the file
    sent: Name of a city where it came from
    finish: Name of the city where it needs to go
    volume: how much the parcel takes up the volume
    """
    parcel_id: int
    volume: int
    sent: str
    destination: str

    def __init__(self, parcel_id: int, volume: int, sent: str, destination: str
                 ) -> None:
        """
        Initialize its parcel with following, id, where its sent, final
        destination and how much it takes up the volume.

        """
        self.parcel_id = parcel_id
        self.sent = sent
        self.destination = destination
        self.volume = volume


class Truck:
    """"
    Truck objects
    ===== Public Attributes =====
    truck_id: Individual ID for each truck
    truck_volume: Max capcaity of how much it can hold
    starting_point: Starting position of the truck
    parcels: storage for parcels.
    """
    truck_id: int
    truck_volume: int
    starting_point: str
    parcels: List[Parcel]

    def __init__(self, truck_id: int, truck_volume: int,
                 starting_point: str) -> None:
        """Initialize its object with distinguished id and volume
        >>> t = Truck(1345,3000,'Toronto')
         """
        self.truck_id = truck_id
        self.truck_volume = truck_volume
        self.starting_point = starting_point
        self.parcels = []

    def fullness(self) -> float:
        """
        Return the total percentage of how much volume of parcels have been
        filled up to its capacity
        >>> f = Fleet()
        >>> t1 = Truck(1423, 10, 'Toronto')
        >>> f.add_truck(t1)
        >>> p1 = Parcel(1, 5, 'Buffalo', 'Hamilton')
        >>> t1.pack(p1)
        True
        >>> p2 = Parcel(2, 4, 'Toronto', 'Montreal')
        >>> t1.pack(p2)
        True
        >>> t1.fullness()
        90.0
        """
        total_volume = 0
        percentage = 0
        for parcel in self.parcels:
            total_volume += parcel.volume
            percentage = total_volume / self.truck_volume
        return float((percentage * 100))

    def pack(self, parcel: Parcel) -> bool:
        """
        Return boolean if parcel is packed in the Truck
        >>> t = Truck(1423, 1000, 'Toronto')
        >>> p = Parcel(1, 5, 'Buffalo', 'Hamilton')
        >>> t.pack(p)
        True
        >>> t1 = Truck(1423, 1000, 'Toronto')
        >>> p2 = Parcel(1, 5, 'Buffalo', 'Hamilton')
        >>> t1.pack(p2)
        True
        """
        if self.parcels == []:
            self.parcels.append(parcel)
            return True
        else:
            self.parcels.append(parcel)
            return True


class Fleet:
    """ A fleet of trucks for making deliveries.

    ===== Public Attributes =====
    trucks:
      List of all Truck objects in this fleet.
    """
    trucks: List[Truck]

    def __init__(self) -> None:
        """Create a Fleet with no trucks.

        >>> f = Fleet()
        >>> f.num_trucks()
        0
        """
        self.trucks = []

    def add_truck(self, truck: Truck) -> None:
        """Add <truck> to this fleet.

        Precondition: No truck with the same ID as <truck> has already been
        added to this Fleet.

        >>> f = Fleet()
        >>> t = Truck(1423, 1000, 'Toronto')
        >>> f.add_truck(t)
        >>> f.num_trucks()
        1
        """
        return self.trucks.append(truck)

    # We will not test the format of the string that you return -- it is up
    # to you.
    def __str__(self) -> str:
        """
        Produce a string representation of this fleet
        """
        # TODO: Complete this method.

    def num_trucks(self) -> int:
        """Return the number of trucks in this fleet.

        >>> f = Fleet()
        >>> t1 = Truck(1423, 10, 'Toronto')
        >>> f.add_truck(t1)
        >>> f.num_trucks()
        1
        """
        count = 0
        for _ in self.trucks:
            count += 1
        return count

    def num_nonempty_trucks(self) -> int:
        """Return the number of non-empty trucks in this fleet.

        >>> f = Fleet()
        >>> t1 = Truck(1423, 10, 'Toronto')
        >>> f.add_truck(t1)
        >>> p1 = Parcel(1, 5, 'Buffalo', 'Hamilton')
        >>> t1.pack(p1)
        True
        >>> p2 = Parcel(2, 4, 'Toronto', 'Montreal')
        >>> t1.pack(p2)
        True
        >>> t1.fullness()
        90.0
        >>> t2 = Truck(5912, 20, 'Toronto')
        >>> f.add_truck(t2)
        >>> p3 = Parcel(3, 2, 'New York', 'Windsor')
        >>> t2.pack(p3)
        True
        >>> t2.fullness()
        10.0
        >>> t3 = Truck(1111, 50, 'Toronto')
        >>> f.add_truck(t3)
        >>> f.num_nonempty_trucks()
        2
        """
        empty = 0

        for truck in self.trucks:

            if truck.fullness() < truck.truck_volume:
                empty += 1

        return empty

    def parcel_allocations(self) -> Dict[int, List[int]]:
        """Return a dictionary in which each key is the ID of a truck in this
        fleet and its value is a list of the IDs of the parcels packed onto it,
        in the order in which they were packed.

        >>> f = Fleet()
        >>> t1 = Truck(1423, 10, 'Toronto')
        >>> p1 = Parcel(27, 5, 'Toronto', 'Hamilton')
        >>> p2 = Parcel(12, 5, 'Toronto', 'Hamilton')
        >>> t1.pack(p1)
        True
        >>> t1.pack(p2)
        True
        >>> t2 = Truck(1333, 10, 'Toronto')
        >>> p3 = Parcel(28, 5, 'Toronto', 'Hamilton')
        >>> t2.pack(p3)
        True
        >>> f.add_truck(t1)
        >>> f.add_truck(t2)
        >>> f.parcel_allocations() == {1423: [27, 12], 1333: [28]}
        True
        """
        result = {}
        for truck in self.trucks:
            for parcel in truck.parcels:
                if truck.truck_id in result:
                    result[truck.truck_id].append(parcel.parcel_id)
                else:
                    result[truck.truck_id] = [parcel.parcel_id]
        return result

    def total_unused_space(self) -> int:
        """Return the total unused space, summed over all non-empty trucks in
        the fleet.
        If there are no non-empty trucks in the fleet, return 0.

        >>> f = Fleet()
        >>> f.total_unused_space()
        0
        >>> t = Truck(1423, 1000, 'Toronto')
        >>> p = Parcel(1, 5, 'Buffalo', 'Hamilton')
        >>> t.pack(p)
        True
        >>> f.add_truck(t)
        >>> f.total_unused_space()
        995
        """
        total_parcel = 0
        space = 0
        for truck in self.trucks:

            if truck.fullness() < truck.truck_volume:
                for parcel in truck.parcels:
                    total_parcel += parcel.volume
                    space = (truck.truck_volume-total_parcel)

        return space

    def _total_fullness(self) -> float:
        """Return the sum of truck.fullness() for each non-empty truck in the
        fleet. If there are no non-empty trucks, return 0.

        >>> f = Fleet()
        >>> f._total_fullness() == 0.0
        True
        >>> t = Truck(1423, 10, 'Toronto')
        >>> f.add_truck(t)
        >>> f._total_fullness() == 0.0
        True
        >>> p = Parcel(1, 5, 'Buffalo', 'Hamilton')
        >>> t.pack(p)
        True
        >>> t1 = Truck(1423, 100, 'Toronto')
        >>> f.add_truck(t1)
        >>> p1 = Parcel(1, 20, 'Buffalo', 'Hamilton')
        >>> p2 = Parcel(2, 30, 'Toronto', 'Montreal')
        >>> t1.pack(p1)
        True
        >>> t1.pack(p2)
        True
        >>> f._total_fullness()
        100.0
        """
        total_full = float(0)

        for truck in self.trucks:

            truck_percentage = ((truck.truck_volume / truck.truck_volume) * 100)
            if truck.fullness() < truck_percentage:
                total_full += truck.fullness()

        return total_full

    def average_fullness(self) -> float:
        """Return the average percent fullness of all non-empty trucks in the
        fleet.

        Precondition: At least one truck is non-empty.

        >>> f = Fleet()
        >>> t = Truck(1423, 10, 'Toronto')
        >>> p = Parcel(1, 5, 'Buffalo', 'Hamilton')
        >>> t.pack(p)
        True
        >>> f.add_truck(t)
        >>> f.average_fullness()
        50.0
        """
        avg_full = 0
        count = 0
        for truck in self.trucks:
            count += 1
            truck_percentage = ((truck.truck_volume / truck.truck_volume) * 100)
            if truck.fullness() < truck_percentage:
                avg_full += truck.fullness()
        return avg_full / count

    def total_distance_travelled(self, dmap: DistanceMap) -> int:
        """Return the total distance travelled by the trucks in this fleet,
        according to the distances in <dmap>.

        Precondition: <dmap> contains all distances required to compute the
                      average distance travelled.

        >>> f = Fleet()
        >>> t1 = Truck(1423, 10, 'Toronto')
        >>> p1 = Parcel(1, 5, 'Toronto', 'Hamilton')
        >>> t1.pack(p1)
        True
        >>> t2 = Truck(1333, 10, 'Toronto')
        >>> p2 = Parcel(2, 5, 'Toronto', 'Hamilton')
        >>> t2.pack(p2)
        True
        >>> from distance_map import DistanceMap
        >>> m = DistanceMap()
        >>> m.add_distance('Toronto', 'Hamilton', 9)
        >>> f.add_truck(t1)
        >>> f.add_truck(t2)
        >>> f.total_distance_travelled(m)
        36
        """
        distance = 0

        for truck in self.trucks:
            for parcel in truck.parcels:
                distance += (dmap.distance(truck.starting_point,
                                           parcel.destination) * 2)

        return distance

    def average_distance_travelled(self, dmap: DistanceMap) -> float:
        """Return the average distance travelled by the trucks in this fleet,
        according to the distances in <dmap>.

        Include in the average only trucks that have actually travelled some
        non-zero distance.

        Preconditions:
        - <dmap> contains all distances required to compute the average
          distance travelled.
        - At least one truck has travelled a non-zero distance.

        >>> f = Fleet()
        >>> t1 = Truck(1423, 10, 'Toronto')
        >>> p1 = Parcel(1, 5, 'Toronto', 'Hamilton')
        >>> t1.pack(p1)
        True
        >>> t2 = Truck(1333, 10, 'Toronto')
        >>> p2 = Parcel(2, 5, 'Toronto', 'Hamilton')
        >>> t2.pack(p2)
        True
        >>> from distance_map import DistanceMap
        >>> m = DistanceMap()
        >>> m.add_distance('Toronto', 'Hamilton', 9)
        >>> f.add_truck(t1)
        >>> f.add_truck(t2)
        >>> f.average_distance_travelled(m)
        18.0
        """
        total_truck = 0
        distance = 0

        for truck in self.trucks:
            total_truck += 1
            for parcel in truck.parcels:

                distance += (dmap.distance(truck.starting_point,
                                           parcel.destination) * 2)

        return distance / total_truck


if __name__ == '__main__':
    import python_ta
    python_ta.check_all(config={
        'allowed-import-modules': ['doctest', 'python_ta', 'typing',
                                   'distance_map'],
        'disable': ['E1136'],
        'max-attributes': 15,
    })
    import doctest
    doctest.testmod()
