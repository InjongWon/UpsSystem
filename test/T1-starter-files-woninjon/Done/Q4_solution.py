...
class LinkedList:
    ...
    def repeat_greater_than_first(self) -> None:
        ...
        if curr is not None:
            first = curr.item
        while curr is not None:  # this line fixes bug
            if curr.item > first:
...
def test_demo_pass():
    # tests case when doubled digit occurs in the middle of LL
    lst = LinkedList([1, 2, 0])
    lst.repeat_greater_than_first()
    assert str(lst) == '[1 -> 2 -> 2 -> 0]'


def test_demo_fail():
    # tests case when the doubled digit occurs at end of LL
    lst = LinkedList([1, 2])
    lst.repeat_greater_than_first()
    assert str(lst) == '[1 -> 2 -> 2]'
