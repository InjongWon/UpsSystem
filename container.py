...
# Do not add any more attributes bo th public and Private!!
    ...
    def add(self, item: Any) -> None:
        ...
        >>> pq._queue
        ['aaaaaaaa', 'monalisa', 'ieeeee', 'ieeee', 'arju', 'fred', 'hat', 'ie']
        >>> pq.remove()
        'ie'
        >>> pq.remove()
        'hat'
        >>> pq._queue
        ['aaaaaaaa', 'monalisa', 'ieeeee', 'ieeee', 'arju', 'fred']
        """

        if self._queue == []:
            self._queue.append(item)
        else:
            for char in self._queue:
                if self._higher_priority(item, char) is True:
                    self._queue.append(item)
                    self._queue.sort(key=len, reverse=True)
                    break
                elif len(char) == len(item):
